...
You can find all of the configured `username:password` credentials with the following command:

.Show all `username:password` combinations from `credentials.json`
----
jq --raw-output '.[] | .identity+":"+.credential' etc/credentials.json
----

Example for sending messages from command line when using example configurations where `security.authRequired` is set to `true`:

.Example with Authorization header dynamically calculating base64 encoded example credentials
----
curl localhost:8080 --header "Authorization: Basic $(echo -n ExampleUser:ExamplePassword | base64)" --data "Example Message"
----

.Example with Authorization header using pre-calcucated base64 encoded example credentials
----
curl localhost:8080 --header "Authorization: Basic RXhhbXBsZVVzZXI6RXhhbXBsZVBhc3N3b3Jk" --data "Example Message"
----

The `Authorization` header can be omitted when `security.authRequired` is set to `false`:

.Example without an Authorization header
----
curl localhost:8080 --data "Example Message"
----
